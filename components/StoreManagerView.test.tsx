...
import { Role, type ProductSKU, type Store, type User } from '../types';
...
const mockStore: Store = { id: 101, name: 'GreenGrocer Downtown', region: 'North' };
...
const mockJustificationReasons: string[] = ['Supplier Delay', 'Seasonal Demand Spike'];
...
describe('StoreManagerView Component', () => {
  ...
  it('renders loading state initially then displays recommendations', async () => {
    render(<StoreManagerView store={mockStore} products={mockProducts} justificationReasons={mockJustificationReasons} />);
  ...
  it('filters recommendations by category', async () => {
      ...
      const categoryFilter = selects.find(s => s.id === 'category-filter');
